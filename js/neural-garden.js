...
( < #}>===   ''   ( _|_ |_ _ _
 ~ ¯ '       /                 \\__        _
  ...
  function milestoneLevel() {
    ...
    if (cc >= 30 || min >= 65)  return Math.min(11, STAGES.length - 1);
    if (cc >= 25 || min >= 45) return Math.min(10, STAGES.length - 1);
    if (cc >= 20 || min >= 30) return Math.min(9,  STAGES.length - 1);
    if (cc >= 16 || min >= 20) return Math.min(8,  STAGES.length - 1);
    if (cc >= 12 || min >= 10)  return Math.min(7,  STAGES.length - 1);
    if (cc >= 9  || min >= 6)  return Math.min(6,  STAGES.length - 1);
    if (cc >= 6)               return 5;
    if (cc >= 3)               return 4;
    if (cc >= 1)               return 3;
    if (nc >= 2)               return 2;
